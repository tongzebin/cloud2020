...
//        log.info("***插入结果" + result);
        if (result > 0) {
            return new CommonResult(200, "插入数据成功服务端口号"+serverPort, result);
        } else {
            return new CommonResult(404, "插入数据失败服务端口号"+serverPort, null);
        }
    }
...
//        log.info("***查询结果" + payment);
        if (null != payment) {
            ...
            return new CommonResult(404, "查询ID为"+id+"的数据失败 服务端口号"+serverPort, null);
        }
    }
}
